...
uint64_t Odometry_t::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == Odometry_t::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, Odometry_t::getHash };

    uint64_t hash = 0xa62aacefbb44620aLL +
